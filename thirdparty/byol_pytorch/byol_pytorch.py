...
class BYOL(nn.Module):
    ...
    def forward(
        ...
        outputs = dict(online_proj_one=online_proj_one, online_proj_two=online_proj_two,
                       online_pred_one=online_pred_one, online_pred_two=online_pred_two,
                       loss=loss.mean())
        return outputs
    ...
    def evaluate(self, x):
        ...
        image_size = 32
        evaluate_augment = torch.nn.Sequential(
            T.RandomResizedCrop((image_size, image_size)),
            T.Normalize(
                mean=torch.tensor([0.485, 0.456, 0.406]),
                std=torch.tensor([0.229, 0.224, 0.225])),
        ...
        image = evaluate_augment(x)

        online_proj, _ = self.online_encoder(image)
        online_pred = self.online_predictor(online_proj)

        outputs = dict(online_proj=online_proj, online_pred=online_pred)
